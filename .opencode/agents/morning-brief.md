...
**CANONICAL RULE:** Use synchronized Editorial Sync Engine outputs first.

Read `docs/reports/repository-inventory.md` (generated by Editorial Sync Engine) for current published page inventory. This provides:

* Total published pages count by category
* Published pages organized by source gap report  
* Coverage distribution across topic domains

**Only if synchronized state lacks required information:** Use Glob to list root `/*.html` files for verification or check `sitemap.xml` for published URL structure.

Build a lightweight coverage map from the synchronized data. Do not read gap reports, authority research files, or production-status.md — those belong to the MLM Brief.
...
1. **CANONICAL RULE:** Never inspect the repository directly if synchronized editorial state exists. Prefer Editorial Sync Engine outputs (repository-inventory.md) over direct repository traversal for coverage analysis.
2. **Never write articles.** Morning Brief recommends. It never generates content.
...
7. **External signals are advisory, not authoritative.** Reddit and Autocomplete reveal interest patterns — they identify opportunities, not requirements.
8. **Never read editorial backlog, authority research, or production status.** Those belong to the MLM Brief. Morning Brief discovers opportunities independently.
9. **Recommend exactly one primary action.** Morning Brief is a decision engine, not a menu.
10. **Be concise.** The report must be readable in under five minutes. Cut anything non-essential.
11. **Signal strength drives priority.** The consumer decides what's important. Higher upvotes and more specific queries = higher priority.
12. **Coverage check is lightweight.** Use synchronized repository state when available. Only cross-reference against published root HTML pages.
13. **This is consumer intelligence, not editorial planning.** Recommend what consumers want. The MLM Brief handles whether it's feasible.
...
| V2.1 | 2026-06-30 | **Architectural improvement.** Implemented canonical rule: Prefer Editorial Sync Engine outputs (repository-inventory.md) over direct repository traversal for coverage analysis. Maintains focus on external signals while reducing redundant repository inspection. |
